...
def order_by_letter(string_list, letter_pos):
    ...
    letter_pos: int, the index for the letter uses to be sort ascendingly

    return
    ------
    sorted list [....]
    """
    # know the 26 fixed letter domain
    # creates a list of 26 empty list for placeholders
    buckets = [[] for dummy_idx in range(NUM_CHARS)]
    for string in string_list:
        # placing each string into their respective placeholder
        char_index = ord(string[letter_pos]) - ord("a")
        buckets[char_index] += [string]
    
    # create the finally sorted, 'combined' list
    answer = []
...
def run_example():
    ...
    """
    string_length = 3
    test_list = [  # random same size characater string
        "".join([random.choice(CHARACTERS) for _ in range(string_length)])
